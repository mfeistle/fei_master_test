...
vu8  DMX_RX_Buff[MAX_CHANNEL_COUNT];
vu32 DMX_StatusRead = 0;
vu16 DMXChannelCount = 0;
...
// Fehler
    ...
    else if (!fDMX_DataCorruption)
    {
      ...
      {
        DMX_RX_Buff[DMXChannelCount] = USART_ReceiveData(USART1);
        DMXChannelCount++;
      }
      else
      {
  ...
  USART_ClearITPendingBit(USART1, USART_IT_FE);  // Framing Error Flag
  USART_ReceiveData(USART1); // ??
  USART_ClearITPendingBit(USART1, USART_IT_RXNE);// Register not empty Flag
}
