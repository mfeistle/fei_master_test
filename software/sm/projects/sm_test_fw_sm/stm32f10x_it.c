...
static vu8 PIR_ENABLE = 0; // Local flag indicating that PIR triggered

vu8 PIR_FLAG,PIR_FLAG2 = 0; // Global flag signalling that PIR triggered
//vu8 ON_OFF_Timer = 0;
vu8 SETUP_Timer = 0;
vu8 DMX_ReceiveMode;
...
static u8  ACinput;
vu8 IN2OC_asserted; // 0: not asserted, otherwise asserted
  ...
  */
  ...
  if (ACinput && !GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_13)) {
    ...
    if(ACphaseTimer<150) {
      ...
      SYS.ACZeroCrossing=REL_SWITCH_PHASE + ACphaseTimer/2 - SYS.ACperiod/4; // dynamic compensation
      if(SYS.ACZeroCrossing>200) SYS.ACZeroCrossing=0; // prevent overflow / sign error
    }
  }
  ...
  switch(Counter_100us)
  {
    ...
    case 3:
      // 230V push button
      if (GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_3) == 0) // Button pushed?
      {
        IN2OC_asserted = 22; // 22 x 1ms = 22ms > 1/50Hz
      }
