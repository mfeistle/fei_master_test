...
typedef enum
{
  ...
  sKNXSET,
  sKNXSET_WAITFORRESPONSE,
  sKNXGET,
  sKNXGET_WAITFORRESPONSE,
...
//static 
uint16_t TimeOutCounter,KNXTimeOutCounter,WaitTimer;
#define TIMEOUT 10
...
void MB_Process1ms(void)
{
  ...
  if (timerMS >= 1000)
    timerMS = 0;
  timeoutMS=timerMS+1; // Timeout for answers is set for 9msec
  ...
  do
  {
    switch (mbState)
    {
      ...
      case sREADY:
      {
        if (timerMS % 20 == 0) // Main Communication Rate for Slider and AAW  20=20ms at 0 20 40 etc msec
        {
          switch(mbModus) {
            case mNONE:
              break;
        ...
        else if (((timerMS+10) % 20) == 0) // Alternate Communication Rate at 10 30 50 etc msec
        {
          if (timerMS % 990 == 0) {
            mbState = sPCONLINE;
            skipTick = 1;
          } else if(((timerMS+30)) % 40 == 0) { // 40msec interval for KNX Communication
            mbState = sKNXSET;
            skipTick = 1;
            if(KNX.bOKTimer) 
              KNX.bOKTimer--;
      ...
      case sAWGETSET:
      {
        ...
        RS_TX_Buffer[RS_TX_Counter++] = 1 & 0xff; // Anzahl Register
        RS_TX_Buffer[RS_TX_Counter++] = 2;  // Byte Count
        RS_TX_Buffer[RS_TX_Counter++] = LocalDaliCh1; 
        RS_TX_Buffer[RS_TX_Counter++] = LocalDaliCh2; 
        crc = CalcCRC16(RS_TX_Buffer, RS_TX_Counter);
        RS_TX_Buffer[RS_TX_Counter++] = crc & 0xFF;
        RS_TX_Buffer[RS_TX_Counter++] = crc >> 8;
        RS_TX_Send(RS_TX_Buffer, RS_TX_Counter);
      ...
      case sKNXGET:
      {
        ...
        // Get slider values
        RS_TX_Counter = 0;
        RS_TX_Buffer[RS_TX_Counter++] = dKNX;
        RS_TX_Buffer[RS_TX_Counter++] = fcREADHOLDINGREGISTER;
        RS_TX_Buffer[RS_TX_Counter++] = KNX_A_Mode >> 8;
        RS_TX_Buffer[RS_TX_Counter++] = KNX_A_Mode & 0xff;
        RS_TX_Buffer[RS_TX_Counter++] = 2 >> 8;   // Anzahl zu lesende 16bit Register high Byte
        RS_TX_Buffer[RS_TX_Counter++] = 2 & 0xff; // Anzahl zu lesende 16bit Register low Byte
        crc = CalcCRC16(RS_TX_Buffer, RS_TX_Counter);
        RS_TX_Buffer[RS_TX_Counter++] = crc & 0xFF;
        ...
        mbState = sKNXGET_WAITFORRESPONSE;
        skipTick = 0;
  
        break;
      }
      case sKNXGET_WAITFORRESPONSE:
      {
      ...
      case sKNXSET:
      {
        ...
        RS_TX_Buffer[RS_TX_Counter++] = dKNX;
        RS_TX_Buffer[RS_TX_Counter++] = fcWRITEMULTIPLEREGISTER;
        RS_TX_Buffer[RS_TX_Counter++] = KNX_A_LuxSensor1 >> 8;
        RS_TX_Buffer[RS_TX_Counter++] = KNX_A_LuxSensor1 & 0xff;
        RS_TX_Buffer[RS_TX_Counter++] = 2 >> 8;
        RS_TX_Buffer[RS_TX_Counter++] = 2 & 0xff; // Anzahl Register
        RS_TX_Buffer[RS_TX_Counter++] = 4; // byte Count
        RS_TX_Buffer[RS_TX_Counter++] = LAMP_Ch1.wLux >> 8;
        RS_TX_Buffer[RS_TX_Counter++] = LAMP_Ch1.wLux & 0xff;
        RS_TX_Buffer[RS_TX_Counter++] = LAMP_Ch2.wLux >> 8;
        RS_TX_Buffer[RS_TX_Counter++] = LAMP_Ch2.wLux & 0xff;
        crc = CalcCRC16(RS_TX_Buffer, RS_TX_Counter);
        RS_TX_Buffer[RS_TX_Counter++] = crc & 0xFF;
        RS_TX_Buffer[RS_TX_Counter++] = crc >> 8;
        RS_TX_Send(RS_TX_Buffer, RS_TX_Counter);
  
        mbState = sKNXSET_WAITFORRESPONSE;
        skipTick = 0;
  
        break;
      }
      
      case sKNXSET_WAITFORRESPONSE:
      {
        if ((RS_RX_Counter != 8) ||
            (RS_RX_Buffer[0] != dKNX) ||
            ((t_mbFunctionCode)RS_RX_Buffer[1] != fcWRITEMULTIPLEREGISTER) ||
            (RS_RX_Buffer[2] !=  KNX_A_LuxSensor1 >> 8) ||
            (RS_RX_Buffer[3] != (KNX_A_LuxSensor1 & 0xff)) ||
            (RS_RX_Buffer[4] != 2 >> 8) ||
            (RS_RX_Buffer[5] != 2 & 0xff) ||
            (CheckCRC16(RS_RX_Buffer, 8) != 1)
           )
      ...
      case sPC_GETFLASHVAR_WAITFORRESPONSE:
      {
        ...
        Flash.Var[indexFlashVar]=Setup.ulVar;
        ...
        // next PC communication faster (ie about PC_DELAY msec)
        if(timerMS>PC_DELAY)
          timerMS-=PC_DELAY;
        else 
          timerMS+=(1000-PC_DELAY);
        break;
      }
      ...
      case sPCONLINE_WAITFORRESPONSE:
      {
        ...
        switch(abPCcommand[0]) {
          ...
          case 0x43:
            ...
            Setup.ulVar=Flash.Var[indexFlashVar];
          ...
          case 0x62: 
          case 0x63:
            indexFlashVar=(abPCcommand[1] & 3); // ((abPCcommand[0] & 3) << 8)
            if(indexFlashVar==0) { // Return FW Information of TB12000
              Setup.abVar[0]=fw_project[abPCcommand[1]];
              Setup.abVar[1]=fw_project[abPCcommand[1]+1];
              Setup.abVar[2]=fw_project[abPCcommand[1]+2];
              Setup.abVar[3]=fw_project[abPCcommand[1]+3];
            }
            mbState=sPC_RETURNFLASHVAR;
            break;
          case 0x80: 
